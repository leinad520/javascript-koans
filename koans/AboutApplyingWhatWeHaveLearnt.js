...
describe("About Applying What We Have Learnt", function() {
  ...
  it("given I'm allergic to nuts and hate mushrooms, it should find a pizza I can eat (functional)", function() {
    ...
    var filtered = _(products).filter(
      el =>
        el.containsNuts === false &&
        _(el.ingredients).all(x => x !== "mushrooms")
    );

    filtered.forEach(product => productsICanEat.push(product));
  ...
  it("should add all the natural numbers below 1000 that are multiples of 3 or 5 (functional)", function() {
    var sum =
      _.range(0, 1000, 3).reduce((a, b) => a + b) +
      _.range(0, 1000, 5).reduce((a, b) => a + b) -
      _.range(0, 1000, 15).reduce(
        (a, b) => a + b
  ...
  it("should count the ingredient occurrence (functional)", function() {
    ...
    let occurrences = _(products)
      .chain()
      .map(product => product.ingredients)
      .flatten()
      .reduce((final, ingredient) => {
        final[ingredient] = (final[ingredient] || 0) + 1;
        return final;
      }, {})
      .value();

    Object.assign(ingredientCount, occurrences);
